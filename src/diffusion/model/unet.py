"""
Adapted from https://github.com/filipbasara0/simple-diffusion/blob/main/simple_diffusion/model/unet.py
"""
...
from ._attention import LinearAttention, Attention, LayerNorm


def get_downsample_layer(in_dim, hidden_dim, is_last):
    if not is_last:
        return nn.Sequential(Rearrange("b c (n p) -> b (c p) n", p=2), nn.Conv1d(in_dim * 2, hidden_dim, 1))
    else:
        return nn.Conv1d(in_dim, hidden_dim, 3, padding=1)
...
def get_upsample_layer(in_dim, hidden_dim, is_last):
    if not is_last:
        return nn.Sequential(nn.Upsample(scale_factor=2, mode="nearest"), nn.Conv1d(in_dim, hidden_dim, 3, padding=1))
    else:
        return nn.Conv1d(in_dim, hidden_dim, 3, padding=1)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, temb_channels, kernel_size=3, stride=1, padding=1, groups=4):
        ...
        self.residual_conv = (
            nn.Conv1d(in_channels, out_channels=out_channels, kernel_size=1)
...
class UNet(nn.Module):
    def __init__(self, hidden_dims=[16, 32], input_size=38, use_linear_attn=False):
        super(UNet, self).__init__()

        self.in_channels = 1  # since table data is 1D
        ...
        self.init_conv = nn.Conv1d(self.in_channels, out_channels=hidden_dims[0], kernel_size=3, stride=1, padding=1)
        ...
        self.conv_out = nn.Conv1d(hidden_dims[0], out_channels=self.in_channels, kernel_size=1)
